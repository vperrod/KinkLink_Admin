...
interface AvatarProps {
  src?: string | null; // allow empty
  alt?: string;
  name?: string; // <-- add name for initials
  size?: "xsmall" | "small" | "medium" | "large" | "xlarge" | "xxlarge";
  status?: "online" | "offline" | "busy" | "none";
}

const sizeClasses = {
  xsmall: "h-6 w-6 text-[10px]",
...
const Avatar: React.FC<AvatarProps> = ({
  ...
  return (
    <div
      className={`relative flex items-center justify-center rounded-full overflow-hidden bg-brand-500 text-white font-semibold ${sizeClasses[size]}`}
    >
      {/* Image */}
      {!showFallback && (
        <img
